...
def enter_license():
    ...
    h = hashlib.sha256(bUsername_trial).hexdigest()[1:9]
    indices = [4, 5, 3, 6, 2, 7, 1, 8]
    reordered_h = [h[i - 1] for i in indices]
    user_key = "picoCTF{1n_7h3_|<3y_of_" + ''.join(reordered_h) + "}"
...
def check_key(key, username_trial):
    ...
    if len(key) != len(key_full_template_trial):
        ...
        for c in key_part_static1_trial:
            if key[i] != c:
                # print("Bro copy ta garna sika yrr.")
                return False
